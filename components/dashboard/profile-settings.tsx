...
export function ProfileSettings({ user, profile }: ProfileSettingsProps) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await supabase.from("companies").update(updates).eq("id", companyInfo.id).select() // IMPORTANT: Add .select() to get back data or a more detailed error
      ...
      toast({
        ...
        description:
          err.message ||
          `Error code: ${err.code}` ||
          "An unknown database error occurred. Check the console for details.",
        variant: "destructive",
      })
    } finally {
      setIsUpdating(false)
    ...
    <form onSubmit={handleUpdate} className="grid gap-6">
      <Card className="bg-[#1A1A1A] border-[#2A2A2A]">
        <CardHeader>
          ...
          <CardDescription className="text-[var(--dashboard-text-color)]/60">
            Manage your company details and connect your bot's database.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="companyName" className="text-[var(--dashboard-text-color)]/80">
              Company Name
            </Label>
            <Input
              id="companyName"
          ...
          <div className="space-y-2">
            <Label htmlFor="supabaseUrl" className="text-[var(--dashboard-text-color)]/80">
              Supabase URL
            </Label>
            <Input
              id="supabaseUrl"
          ...
          <div className="space-y-2">
            <Label htmlFor="supabaseAnonKey" className="text-[var(--dashboard-text-color)]/80">
              Supabase Anon Key
            </Label>
            <Input
              id="supabaseAnonKey"
